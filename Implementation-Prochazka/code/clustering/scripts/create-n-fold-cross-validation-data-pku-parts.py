...
def create_data_files_pku(actions_file: str, data_file: str, model: str) -> None:
    actions_to_keep: set[str] = set()
    ...
    write_output_to_file(f"{data_file}-{model}-train", fold_data)
    ...
    for beta in BETAS:
        for model in MODELS:
            create_data_files_pku(
                ...
                f"/storage/brno12-cerit/home/drking/experiments/SCL-segmented-actions-norm/pku-mmd/cs/lat_dim={dim}_beta={beta}/predictions_segmented_model_norm={model}.data",
                "cs",
            )
