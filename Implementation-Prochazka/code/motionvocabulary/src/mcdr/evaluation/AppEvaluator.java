...
//        String hullDir = "K:/research/motion-words/hulls/hdm05-actions-hulls-closest";
        ...
        //readSegmentQueriesAsActions(queryMgmt, queryFile, segmentClass);
        readSegmentQueriesAsMotions(queryMgmt, queryFile, segmentClass);
...
//              ObjectMultiCategoryClassifier objectClassifier = new ObjectTrainingSampleRatioClassifier(dataMgmt);
        ...
        // Evaluation
        if (maxK == null) {
            // Search app -- precision of kNN queries where k is set to the size of category into which the query object falls into.
            ...
            // Classification performance -- by kNN classifier (in ECIR we presented 4nn classifier results)
...
//                dataMgmt.saveConfusionMatrixToFile(confMatrix, confMatrixFile, true);
    ...
    private static <E extends LocalAbstractObject> void readSegmentQueriesAsActions(ObjectMgmt queryMgmt, String queryFile, Class<E> queryFileClass) {
        final StreamGenericAbstractObjectIterator<? extends LocalAbstractObject> queryIter = FileUtils.openDB(queryFileClass, queryFile);
        List<E> sgms = new ArrayList<>();
        String action = null;
        while (queryIter.hasNext()) {
