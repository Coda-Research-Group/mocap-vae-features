...
public class DescriptorTester {
    ...
    /**
     * Displays the usage instructions for the SelectPivots command-line tool
     *
     * Usage details are printed to the standard error stream.
     */
    private static void usage() {
        System.err.println("Usage: SelectPivots [parameters]");
        System.err.println("Parameters:");
        System.err.println("            -h [--help]         - show this help, if no -cv/cs args, leave one out validation is applied");
        System.err.println("            -k <k_value>        - k-value for k-nearest neighbor evaluation, default: null");
        System.err.println("            -fp <file_path>     - file path to the feature vector file, default: ");
        System.err.println("            -cv                 - use cross validation to evaluate the k-nearest neighbor and split data as cv, default: false");
        System.err.println("            -cs                 - use cross validation to evaluate the k-nearest neighbor and split data as cs, default: false");
        System.err.println("            -dd                 - dataset categories decription, default: none");
        System.err.println("            --nmatches          - use n-matches instead of hard assignment, default: false");
        System.err.println("            --soft              - use soft assignment instead of hard assignment, default: false");
        System.err.println("            --scl               - use SCL instead of MW, default: false");
        System.err.println("            --base              - use base skeletons L2 + DTW, default: false");
    }
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (int i = 0; i < args.length; ) {
            ...
            if (args[i].equals("--nmatches")) {
                if (++i >= args.length)
                    err = true;
                else {
                    objectClass = SequenceMotionWordsNMatchesDTW.class;
                    ObjectMotionWordNMatches.nMatches = Integer.parseInt(args[i]);
                }
            }
            else if (args[i].equals("--soft")) {
                objectClass = SequenceMotionWordsSoftAssignmentDTW.class;
            }
            else if (args[i].equals("--scl")) {
                objectClass = SequenceSegmentCodeListDTW.class;
            }
            else if (args[i].equals("--base")) {
                objectClass = SequenceMocapPoseCoordsL2DTW.class;
            }
            else if (args[i].equals("-k")) {
                if (++i >= args.length)
                    err = true;
                else {
                    String[] kStrs = args[i].split(",");
                    fixedKsToEvaluate = new int[kStrs.length];
                    for (int j = 0; j < kStrs.length; j++)
                        fixedKsToEvaluate[j] = Integer.parseInt(kStrs[j]);
                }
                ...
                objectLocatorRegExQueryMgmt = ".*-M_.*";
                objectLocatorRegExDataMgmt = ".*-(L|R)_.*";
...
//        Class<? extends SequenceMocap<?>> objectClass = SequenceMocapPoseCoordsL2DTW.class;
...
//            ObjectCategoryMgmt categoryMgmt = new ObjectCategoryMgmt("/home/drking/Documents/bakalarka/mocap-vae-features/Implementation-Prochazka/data/leave-one-out/category_description_short.txt");
            ObjectCategoryMgmt categoryMgmt = new ObjectCategoryMgmt(dataDescription);
...
//                    dataMgmt.saveConfusionMatrixToFile(confMatrix, "/home/drking/Documents/bakalarka/confMatrix.txt", true);
                }
            }
        }
    }
