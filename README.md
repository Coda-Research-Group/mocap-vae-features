...
* **VAE Original Repository**: This work builds upon an original Variational Autoencoder (VAE) implementation. The contributions of this thesis (the MW repository) can be observed by comparing the code difference (diff) between the original repository (branch: original-repository) and this repository (branch main), which is available on GitHub (URL: **<https://github.com/Coda-Research-Group/mocap-vae-features>**).